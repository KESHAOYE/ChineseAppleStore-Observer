  ...
  >
    <el-table :data="task" style="width: 100%">
      <el-table-column type="expand">
        <template slot-scope="props">
          <div class="log_show" :ref="props.row.taskId">
            <div
              class="log_item"
              v-for="(log, key) in props.row.log"
              :key="key"
            >
              <div class="status">
                <i :class="iconDict[log.result.status]"></i>
              </div>
              <div class="info">
                ...
                <div
                  ...
                  v-if="
                    log.result &&
                    (log.result.status === 'available' ||
                      log.result.status === 'unavailable' ||
                      log.result.status === 'ineligible')
                  "
                >
                  {{ log.result.info | result(log.result.status) }}
                </div>
                <div class="result" v-else>
                  信息: {{ log.result && log.result.info }}
                </div>
              </div>
            </div>
          </div>
      ...
      <el-table-column label="城市" width="180">
        <template slot-scope="scope">
          {{
            scope.row.storeInfo.address.stateName
              | location(scope.row.storeInfo.address.city)
          }}
        </template>
      </el-table-column>
...
export default {
  ...
  computed: {
    ...mapState(useAppStore, ["task", "interval"]),
  },

  filters: {
    location(province, city) {
      return province === city ? `${city}` : `${province}省${city}市`;
    },
    status(s) {
      return status[s];
    },
    result(result, s) {
      return s === "available" ? `有货,当前状态为${result}` : `当前暂无货供应`;
    },
  },
  ...
  methods: {
    ...mapActions(useAppStore, ["changeTaskInterval"]),
    ...
    formatTime(el) {
      return setInterval(() => {
        if (["stop", "success", "error"].includes(el.status)) {
          ...
          el.intervalTime = moment
            .utc(Date.now() - new Date(el.beginTime))
            .format("HH:mm:ss");
        }
      }, 1000);
    },
  ...
  mounted() {
    ...
    window.addEventListener("addTask", (e) => {
      const index = this.task.findIndex((el) => el.taskId === e.taskId);
      this.changeTaskInterval([index, this.formatTime(this.task[index])]);
    });
  },
};
</script>
...
.log_show {
  ...
  .log_item {
    ...
    animation: backgroundChange 0.8s linear;
    display: flex;
