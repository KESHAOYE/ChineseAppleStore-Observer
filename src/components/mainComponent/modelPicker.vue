   ...
   <div class="main_choose" v-if="selectTypeId != -1">
    ...
    <div class="choose_property">
      ...
      <div class="choose_models" v-if="models.length > 0">
       ...
       <el-empty description="暂无可选机型" v-else></el-empty>
      <!-- SKU1-一般是颜色 -->
      <span class="tips">外观：<span class="gray">选择你喜欢的颜色</span></span>
      <span class="tips" style="margin:10px 0;font-size:1em"> <span v-if="selectColor !=null">{{this.selectColor}}</span></span>  
        <div class="colorPicker" v-if="skus.length>0">
          <div class="color_ring" v-for="i in skus" :key="i.color" :class="{color_select: i.chineseColor == selectColor}">
            <div class="colors"  @click="createRom(i.id, i.chineseColor)" :style="{background: i.value}"></div>
          </div>
        </div>
        <el-empty description="暂无可选颜色" v-else></el-empty>
       <!--sku2 存储-->
       <span class="tips">存储容量：<span class="gray">你要多大的容量？</span></span>
        <div class="choose_models Picker" v-if="roms.length>0">
          <div class="models rows" :class="{model_select: key == selectRom}" v-for="(i, key) in roms[0]" :key="key" @click="generatorSku(i,key)">{{key}}</div>
        </div>
        <el-empty description="暂无可选容量" v-else></el-empty>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...mapState(['selectTypeId', 'categoryIndex', 'selectSku', 'selectColor', 'selectRom'])
  },
  methods: {
    ...mapMutations(['init', 'changeSelectTypeId', 'changeChooseInfo']),
    // 根据品类ID读取机型系列
    readType() {
