...
async function _runTask(name, useServerChan, useDialogMessage, interval, selectInfo, storeInfo, arr) {
    self.postMessage({
      type: 'vuexTaskValue',
      name,
      task: arr[name]
...
function run(name, useServerChan, useDialogMessage, interval, selectInfo, storeInfo, now) {
  let arr = {}
   now ? _runTask(name, useServerChan, useDialogMessage, interval, selectInfo, storeInfo, arr) : ''
   arr[name] = setInterval(function(){ _runTask(name, useServerChan, useDialogMessage, interval, selectInfo, storeInfo, arr)}, interval * 1000)
}
...
self.addEventListener('message',function(e) {
   ...
   if(data.type == 'beginObserve') {
     run(data.name, data.useServerChan, data.useDialogMessage, data.interval, data.selectInfo, data.storeInfo, data.now)
   } else {
